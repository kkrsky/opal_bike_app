...
class GradlePropertiesParser {
      ...
      if (!value) {
        ...
        if (this._defaults[key] && this._defaults[key] !== properties[key]) {
          ...
          if (key === "org.gradle.jvmargs") {
            shouldEmit = this._isJVMMemoryLessThanRecommended(
              properties[key],
              this._defaults[key]
            );
          }
  ...
  _isJVMMemoryLessThanRecommended(memoryValue, recommendedMemoryValue) {
    ...
    const recommendedBase = this._getBaseJVMSize(
      recommendedCapture[SIZE],
      recommendedCapture[UNIT]
    );
    const memoryCapture = regex.exec(memoryValue);
    const memoryBase = this._getBaseJVMSize(
      memoryCapture[SIZE],
      memoryCapture[UNIT]
    );
  ...
  _getBaseJVMSize(size, unit) {
    ...
    events.emit(
      "warn",
      `[Gradle Properties] Unknown memory size unit (${unit})`
    );
    return null;
  }
