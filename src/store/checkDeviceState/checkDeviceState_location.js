let checkDeviceState_location = {
  init({ state }) {
    console.log("[start] checState_location_init");

    //デバイス状態　location　をチェックする
    if (state.isDiagnostic && state.isDevice) {
      this.locationAuthorizationMode(state);
      this.isLocationAvailable(state);
      this.isLocationEnabled(state);
      this.isLocationAuthorized(state);
      this.getLocationAuthorizationStatus(state);
      this.registerLocationStateChangeHandler(state);
      //requestLocationAuthorization

      if (state.deviceInfo.platform === "android") {
        //android
        this.getLocationMode_android(state);
        this.isGpsLocationAvailable_android(state);
        this.isGpsLocationEnabled_android(state);
        this.isNetworkLocationAvailable_android(state);
        this.isNetworkLocationEnabled_android(state);
        //switchToLocationSettings_android
      }
      if (state.deviceInfo.platform === "ios") {
        //ios
        // commit('locationAccuracyAuthorization_ios')
        this.getLocationAccuracyAuthorization_ios(state);
        this.registerLocationStateChangeHandler_ios(state);
   ...
   */
    ...
    cordova.plugins.diagnostic.getLocationMode(onSuccess, (error) => {
      console.error(error);
    });
  },
  ...
  isGpsLocationAvailable_android(state) {
    ...
    cordova.plugins.diagnostic.isGpsLocationAvailable(onSuccess, (error) => {
      console.error(error);
    });
    // cordova.plugins.diagnostic.isGpsLocationAvailable(onSuccess, this.error);
  },
   ...
   */
    ...
    cordova.plugins.diagnostic.isGpsLocationEnabled(onSuccess, (error) => {
      console.error(error);
    });
  },
   ...
   */
    ...
    cordova.plugins.diagnostic.isNetworkLocationEnabled(onSuccess, (error) => {
      console.error(error);
    });
  },
   ...
   */
    ...
    cordova.plugins.diagnostic.isLocationAvailable(onSuccess, (error) => {
      console.error(error);
    });
  },
  isLocationEnabled(state) {
    ...
    cordova.plugins.diagnostic.isLocationEnabled(onSuccess, (error) => {
      console.error(error);
    });
  },
   ...
   */
    ...
    cordova.plugins.diagnostic.isLocationAuthorized(onSuccess, (error) => {
      console.error(error);
    });
  },
  getLocationAuthorizationStatus(state) {
    ...
    } else if (state.deviceInfo.platform === "ios") {
      cordova.plugins.diagnostic.getLocationAuthorizationStatus(
        onSuccess_ios,
        (error) => {
          console.error(error);
   ...
   */
    ...
    let onSuccess = (state) => {
      if (
        (state.deviceInfo.platform === "android" &&
          state !== cordova.plugins.diagnostic.locationMode.LOCATION_OFF) ||
        (state.deviceInfo.platform === "ios" &&
...
export default checkDeviceState_location;
